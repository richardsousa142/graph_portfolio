...
def main(data, asset):
  ...
  # Ordena ativos conforme a estrutura hierárquica
  ordered_assets = [asset[i] for i in hr.leaves_list(clustering)]
  dict_asset_weight = {a: round(w / 100, 4) for a, w in zip(ordered_assets, vet_weight)}

  return dict_asset_weight, np.array(vet_weight) / 100